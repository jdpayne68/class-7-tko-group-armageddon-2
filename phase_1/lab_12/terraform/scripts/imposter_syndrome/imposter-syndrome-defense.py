...
def run_command_with_wait(
    ...
    """Run a command while showing a simple elapsed-time wait animation."""
    ...
    while process.poll() is None:
        ...
        print(f"\r[{elapsed:02d}s] {' .' * dots}", end="", flush=True)
...
def run_terraform(terraform_dir: Path, tfvars_file: Path, name: str) -> bool:
    """Run Terraform plan and apply using the Chewbacca tfvars file."""
    ...
    Logger.success("Terraform plan succeeded.")
    pause(PAUSE_MEDIUM)
