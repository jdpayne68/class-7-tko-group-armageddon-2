...
These steps manually recreate what `scripts/build-layers.sh` does for the Lab
12b ReportLab layer.
...
The Executive Dashboard Lambda imports `reportlab` to create PDF reports.
`reportlab` is not built into the AWS Lambda Python runtime, so we put it in a
Lambda layer.
...
Start from the Lab 12b root directory. This is the folder that contains
`requirements.txt` and the `terraform/` directory.
...
The `requirements.txt` install prepares your local Python environment. The layer
itself is still built by the later `pip install --target "$SITE_PACKAGES"`
command, because those packages must be copied into the Lambda layer directory.

Move into the Terraform root module:
