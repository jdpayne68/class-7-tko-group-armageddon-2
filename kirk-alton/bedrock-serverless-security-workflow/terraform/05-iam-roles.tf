...
resource "aws_iam_role" "waf_log_processor_role" {
  name               = "${local.name_prefix}-waf-log-processor-role"
  assume_role_policy = data.aws_iam_policy_document.lambda_assume_role.json
  description        = "Execution role for the WAF log forwarder Lambda"
}

resource "aws_iam_role_policy_attachment" "waf_log_processor_basic_execution" {
  role       = aws_iam_role.waf_log_processor_role.name
  policy_arn = "arn:${data.aws_partition.current.partition}:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
}

resource "aws_iam_role_policy_attachment" "waf_log_processor" {
  role       = aws_iam_role.waf_log_processor_role.name
  policy_arn = aws_iam_policy.waf_log_processor.arn
}
