...
# -------------------------------------------------------------------------------
resource "aws_api_gateway_resource" "waf_log_processor" {
  rest_api_id = aws_api_gateway_rest_api.chewbacca_auth_rest_api.id
  parent_id   = aws_api_gateway_rest_api.chewbacca_auth_rest_api.root_resource_id
...
resource "aws_api_gateway_method" "waf_log_processor_post" {
  rest_api_id   = aws_api_gateway_rest_api.chewbacca_auth_rest_api.id
  resource_id   = aws_api_gateway_resource.waf_log_processor.id
  http_method   = "POST"
  authorization = "NONE"

}

resource "aws_api_gateway_integration" "waf_log_processor_lambda" {
  rest_api_id             = aws_api_gateway_rest_api.chewbacca_auth_rest_api.id
  resource_id             = aws_api_gateway_resource.waf_log_processor.id
  http_method             = aws_api_gateway_method.waf_log_processor_post.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.waf_log_processor.invoke_arn
...
resource "aws_api_gateway_deployment" "chewbacca_auth_rest" {
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_resource.waf_log_processor.id,
      aws_api_gateway_method.waf_log_processor_post.id,
      aws_api_gateway_integration.waf_log_processor_lambda.id
  ...
  depends_on = [
    ...
    aws_api_gateway_integration.waf_log_processor_lambda
  ]
}
# -------------------------------------------------------------------------------
# REST API Prod Stage
