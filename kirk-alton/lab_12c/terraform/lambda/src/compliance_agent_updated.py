...
CONTROLS_JSON = os.environ.get("CONTROLS_JSON")
CONTROLS_S3_URI = os.environ.get("CONTROLS_S3_URI")
ALLOW_EMBEDDED_CONTROLS = (
    os.environ.get("ALLOW_EMBEDDED_CONTROLS", "true").lower()
    == "true"
)
...
DEFAULT_CONTROLS_DOCUMENT = {
    "controls": [
        {
            "control_id": "CTRL-001",
            "title": "AWS WAF Protection",
            "description": "API Gateway must be protected by AWS WAF.",
            "category": "Network Security",
            "severity": "High",
            "service": "AWS WAF",
            "resource_type": "WebACL",
            "evidence_sources": ["waf-events"],
            "frameworks": [
                {
            ...
            "validation": {
                "type": "table_exists",
                "table": "WAF_EVENTS_TABLE",
            },
            "bedrock_prompt": (
                "Explain whether evidence demonstrates that AWS WAF is "
                "actively protecting the application."
            ),
        },
        {
            "control_id": "CTRL-002",
            ...
            "description": (
                "Security events should be correlated to identify attack "
                "campaigns."
            ),
            "category": "Detection",
            "severity": "Medium",
            "service": "Threat Correlation Agent",
            "resource_type": "Lambda",
            "evidence_sources": ["waf-correlation-findings"],
            "frameworks": [
                {
                    "framework": "NIST CSF 2.0",
                    "control": "DE.AE-03",
            ...
            "validation": {
                "type": "table_exists",
                "table": "CORRELATION_FINDINGS_TABLE",
            },
            "bedrock_prompt": (
                "Summarize whether correlated findings demonstrate effective "
        ...
        },
        {
            ...
            "description": (
                "Security incidents should be tracked and managed."
            ),
            "category": "Incident Response",
            "severity": "High",
            "service": "SOAR Response Agent",
            "resource_type": "Lambda",
            "evidence_sources": ["security-incidents"],
            "frameworks": [
                {
                    "framework": "NIST CSF 2.0",
                    "control": "RS.MA-01",
            ...
            "validation": {
                "type": "table_exists",
                "table": "SECURITY_INCIDENTS_TABLE",
            },
            "bedrock_prompt": (
                "Determine whether incident response evidence supports the "
                "operation of the response process."
            ),
        },
        {
            "control_id": "CTRL-004",
            ...
            "description": (
                "Security reporting should be available for leadership."
            ),
            "category": "Governance",
            "severity": "Medium",
            "service": "Executive Dashboard Agent",
            "resource_type": "Lambda",
            "evidence_sources": ["executive-security-reports"],
            "frameworks": [
                {
                    "framework": "NIST CSF 2.0",
                    "control": "GV.OV-01",
            ...
            "validation": {
                "type": "s3_prefix",
                "bucket": "EXECUTIVE_REPORT_BUCKET",
                "prefix": "executive-reports",
            },
            "bedrock_prompt": (
                "Explain whether executive reporting demonstrates governance "
...
def log_header(title: str) -> None:
    """Print a compact Lambda log header."""
...
def log_step(message: str) -> None:
    """Print a major workflow step."""

    print(f"[STEP] {message}")


def log_info(message: str) -> None:
    """Print an informational Lambda log line."""

    print(f"[INFO] {message}")


def log_warn(message: str) -> None:
    """Print a warning Lambda log line."""

    print(f"[WARN] {message}")


def log_error(message: str) -> None:
    """Print an error Lambda log line."""

    print(f"[ERROR] {message}")


def parse_json_body(value: Any) -> dict[str, Any]:
    """Parse a Lambda event body when invoked through API Gateway."""
    ...
    if isinstance(value, str):
        ...
        if not isinstance(parsed, dict):
            raise ValueError(
                "Event body JSON must decode to an object."
            )

        return parsed

    raise ValueError(
        "Event body must be a JSON string or object."
    )


def normalize_event(event: Any) -> dict[str, Any]:
    """
    Normalize direct Lambda, scheduler, and API Gateway-style events.

    Body fields override wrapper fields so API callers can pass the same shape
    used by direct Lambda tests.
    """
...
def response(
    ...
    """Return a standard Lambda proxy-compatible response."""
...
def load_controls_from_s3(uri: str) -> dict[str, Any]:
    """Load a controls document from an s3://bucket/key URI."""

    if not uri.startswith("s3://"):
        raise ValueError(
            "CONTROLS_S3_URI must use the s3://bucket/key format."
        )
    ...
    if not bucket or not key:
        raise ValueError(
            "CONTROLS_S3_URI must include both bucket and key."
        )
...
def validate_controls_document(
    document: dict[str, Any],
) -> list[dict[str, Any]]:
    """Validate a controls document and return its controls list."""
    ...
    log_info(f"Loaded {len(controls)} control definition(s).")
...
def load_controls(
    path: str,
    inline_controls: Any = None,
) -> list[dict[str, Any]]:
    """
    Load controls from the most specific available source.

    Priority:
    1. Event-provided controls.
    2. CONTROLS_JSON environment variable.
    3. CONTROLS_S3_URI environment variable.
    4. CONTROLS_FILE path.
    5. Embedded lab defaults.
    """

    if inline_controls is not None:
        log_info("Loading control definitions from invocation event.")

        if isinstance(inline_controls, list):
            return validate_controls_document(
                {"controls": inline_controls}
            )

        if isinstance(inline_controls, dict):
            return validate_controls_document(inline_controls)

        raise ValueError(
            "Event controls must be a list or an object with a controls list."
        )

    if CONTROLS_JSON:
        log_info("Loading control definitions from CONTROLS_JSON.")
        return validate_controls_document(
            json.loads(CONTROLS_JSON)
        )

    if CONTROLS_S3_URI:
        log_info(
            f"Loading control definitions from {CONTROLS_S3_URI}."
        )
        return validate_controls_document(
            load_controls_from_s3(CONTROLS_S3_URI)
        )

    controls_path = Path(path)

    if controls_path.exists():
        log_info(f"Loading control definitions from {controls_path}.")
    ...
    if ALLOW_EMBEDDED_CONTROLS:
        log_warn(
            f"Control library {controls_path} was not found. "
            "Using embedded lab defaults."
        )
        return validate_controls_document(DEFAULT_CONTROLS_DOCUMENT)
...
def lambda_handler(
    ...
    try:
        request_event = normalize_event(event)
        ...
        frameworks = normalize_frameworks(
            request_event.get("frameworks")
        )

        controls_file = str(
            request_event.get("controls_file", CONTROLS_FILE)
        )

        log_info(f"Requested frameworks: {frameworks}")
        log_info(f"Control library: {controls_file}")

        # Stage 1: Load and select controls.
        log_step("Loading and selecting controls")
        all_controls = load_controls(
            path=controls_file,
            inline_controls=request_event.get("controls"),
        )
        ...
        log_step("Evaluating deterministic controls")
        ...
        if ENABLE_BEDROCK:
            try:
                log_step("Generating Bedrock narrative")
                ...
                log_info("Bedrock narrative generated.")
        ...
        log_step("Generating PDF and JSON artifacts")
        ...
        log_info(json.dumps(response_body, indent=2))
