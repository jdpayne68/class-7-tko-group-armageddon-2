...
def convert_to_dynamodb(value: Any) -> Any:
    """
    Convert Python types to DynamoDB-compatible types (Decimal for numbers).
    """
    if isinstance(value, list):
        return [convert_to_dynamodb(item) for item in value]
    
    if isinstance(value, dict):
        return {key: convert_to_dynamodb(item) for key, item in value.items()}
...
def call_bedrock(
    ...
    serializable_package = convert_to_serializable(evidence_package)
...
Evidence:
{json.dumps(serializable_package, indent=2, default=str)}
...
def emit_eventbridge_event(finding_id: str, severity: str, risk_score: int):
    """
    Emit custom EventBridge event for SOAR workflow.
    
    Event format matches the EventBridge rule pattern:
    - Source: seir.waf.correlation
    - Detail-Type: WAF Threat Finding Created
    - Detail: contains finding_id, severity, risk_score
    """
    ...
    try:
        ...
        if response.get('FailedEntryCount', 0) > 0:
            ...
            print(f"Successfully emitted EventBridge event for finding {finding_id}")
            
    except Exception as e:
        print(f"Failed to emit EventBridge event: {e}")
...
def lambda_handler(
    ...
    try:
        ...
        print(
            json.dumps(
                convert_to_serializable(evidence_package),
                indent=2,
                default=str,
            )
        )
        ...
        # Emit EventBridge event for SOAR workflow (only for severities that trigger SOAR)
        if severity in ['MEDIUM', 'HIGH', 'CRITICAL']:
            emit_eventbridge_event(finding_id, severity, risk_score)
            print(f"Emitted EventBridge event for finding {finding_id} with severity {severity}")
        else:
            print(f"No EventBridge event emitted for severity {severity} (LOW findings are recorded but not actioned)")
