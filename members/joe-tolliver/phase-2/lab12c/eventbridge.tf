...
#########################################
# Eventbridge Schedule Executive Agent
#########################################
...
#########################################
# Eventbridge Compliance Agent
#########################################

resource "aws_cloudwatch_event_rule" "compliance_agent" {
  name                = "compliance-agent"
  description         = "Compliance Evidence Agent"
  schedule_expression = "cron(0 23 * * ? *)"
}

resource "aws_cloudwatch_event_target" "compliance_dashboard" {
  rule = aws_cloudwatch_event_rule.compliance_agent.name
  arn  = aws_lambda_function.compliance_evidence_agent.arn
}
...
resource "aws_lambda_permission" "allow_eventbridge_compliance_agent" {
  statement_id  = "AllowExecutionFromEventBridgeComplianceAgent"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.compliance_evidence_agent.function_name
  principal     = "events.amazonaws.com"
  source_arn    = aws_cloudwatch_event_rule.compliance_agent.arn
