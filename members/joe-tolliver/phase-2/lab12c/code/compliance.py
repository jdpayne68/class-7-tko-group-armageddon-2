...
def build_evidence_item(
    ...
    """Build the DynamoDB item for one control's evidence record."""
...
def evaluate_and_record_controls(
    controls: list[dict[str, Any]],
    evaluated_at: datetime,
    report_id: str,
) -> tuple[list[dict[str, Any]], int]:
    """
    Evaluate every selected control and persist its evidence immediately.

    Evidence is written one control at a time, right after that control is
    evaluated -- not batched until the end of the loop. If the Lambda is
    interrupted (timeout, crash, throttling) partway through a long control
    set, every control evaluated up to that point already has a durable
    evidence record in DynamoDB. A write failure for one control is logged
    and does not stop evaluation of the remaining controls.
    """

    table = dynamodb_resource.Table(COMPLIANCE_EVIDENCE_TABLE)
    ...
    for control in controls:
        ...
        try:
            ...
            # A failed write should not take down the rest of the run.
            # The control's result still lives in `results` for the
            # report and score, but the missing evidence record is
            # visible in the logs for follow-up.
            print(
                f"Failed to write evidence for "
                f"{control['control_id']}: "
                f"{type(error).__name__}: {error}"
            )

    return results, evidence_records_written
...
def lambda_handler(
    ...
    try:
        ...
        # Stages 2, 3, and 4: Evaluate each control and persist its
        # evidence record immediately, one control at a time.
        results, evidence_records_written = evaluate_and_record_controls(
        ...
        print(
            f"Wrote {evidence_records_written} of "
            f"{len(results)} evidence record(s) to DynamoDB."
        )
