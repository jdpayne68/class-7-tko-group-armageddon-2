...
class ThreatCorrelation:
    ...
    def correlate(
        ...
        """
        Group evidence that refers to the same indicator.
        """
        ...
        # Group evidence by indicator type + normalized value.
        for item in evidence:
            key = (
                item.indicator_type,
                item.indicator_value.casefold(),
            )
        ...
        # Turn each bucket into a CorrelationGroup.
        for (indicator_type, indicator_value), items in grouped.items():
            groups.append(
                CorrelationGroup(
                    evidence=items,
                    rationale=[
                        (
                            "Evidence grouped because all observations "
                            f"reference the same {indicator_type.value} "
...
class ThreatClassifier:
    ...
    def classify(
        ...
        threat_domains: set[ThreatDomain] = set()

        for evidence in group.evidence:

            threat_type = self.CONDITION_TYPE_MAP.get(
                evidence.condition,
                ThreatType.UNKNOWN,
            )

            threat_domain = self.CONDITION_DOMAIN_MAP.get(
                evidence.condition,
            ...
            if threat_domain != ThreatDomain.UNKNOWN:
                threat_domains.add(threat_domain)

        if len(threat_types) == 1:
            final_type = next(iter(threat_types))
        else:
            final_type = ThreatType.UNKNOWN

        if len(threat_domains) == 1:
            final_domain = next(iter(threat_domains))
        else:
            final_domain = ThreatDomain.UNKNOWN
...
class ThreatAssessmentEngine:
    ...
    def assess(
        ...
        """
        Produce a deterministic threat assessment from
        correlated evidence.
        """

        result = AssessmentResult()

        # Preserve evidence used for the assessment.
        result.supporting_evidence = list(group.evidence)
        ...
        # Classify the correlated evidence.
        classifier = ThreatClassifier()

        (
            result.threat_type,
            result.threat_domain,
        ) = classifier.classify(group)

        # Facts used by our deterministic policy.
        known_exploited = False
        vulnerable_asset = False
        validated_evidence = False
        high_trust_providers = 0

        for evidence in group.evidence:

            if evidence.condition in {
                ThreatCondition.VULNERABLE_LIBRARY,
                ThreatCondition.UNPATCHED_VULNERABILITY,
            }:
                vulnerable_asset = True
            ...
            if (
                evidence.provider_trust
                == ProviderTrustLevel.HIGH
            ):
                high_trust_providers += 1

            provider_data = evidence.metadata.get(
                "provider_data",
                {},
            )

            if provider_data.get("known_exploited") is True:
                known_exploited = True

        # Deterministic assessment policy.
        if known_exploited and vulnerable_asset:
            ...
            result.rationale.append(
                "The vulnerability is listed as known exploited."
            )

            result.rationale.append(
                "Evidence indicates that an affected library "
                "exists in the environment."
            )

            result.rationale.append(
                "Remediation is recommended because external "
                "exploitation intelligence and internal exposure "
                "evidence are correlated."
            )

        elif vulnerable_asset:
            ...
            result.rationale.append(
                "Evidence indicates a vulnerable library exists "
                "in the environment."
            )
            ...
            result.confidence = ThreatConfidence.OBSERVED
            ...
            result.rationale.append(
                "Available evidence does not establish exposure "
                "of an internal asset."
            )

        result.metadata = {
            "known_exploited": known_exploited,
            "vulnerable_asset": vulnerable_asset,
            "high_trust_providers": high_trust_providers,
