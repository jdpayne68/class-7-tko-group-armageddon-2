...
#   tags = var.common_tags
# }
resource "aws_iam_role_policy" "soar_eventbridge_policy" {
  name = "${var.prefix}-soar-eventbridge-policy"
  role = aws_iam_role.soar_role.id
...
resource "aws_iam_role_policy" "soar_bedrock_policy" {
  name = "${var.prefix}-soar-bedrock-policy"
  role = aws_iam_role.soar_role.id
...
# Compliance Agent Lambda Role
resource "aws_iam_role" "compliance_role" {
  name               = "${var.prefix}-compliance-role"
  assume_role_policy = data.aws_iam_policy_document.lambda_assume_role.json
  tags               = var.common_tags
}
...
resource "aws_iam_role_policy" "compliance_policy" {
  name = "${var.prefix}-compliance-policy"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # Inspect DynamoDB tables used as compliance evidence
      {
        Effect = "Allow"
        Action = [
        ...
        Resource = [
          ...
          var.waf_correlation_findings_table_arn,
          var.security_incidents_table_arn,
          var.compliance_evidence_table_arn
        ]
      },

      # Write PASS / FAIL / REVIEW evidence
      {
        Effect = "Allow"
        Action = [
          "dynamodb:PutItem",
        ...
        Resource = var.compliance_evidence_table_arn
      },

      # Inspect existing executive/compliance reports in S3
      {
        Effect = "Allow"
        Action = [
          "s3:ListBucket"
      ...
      # Read/write report objects in the existing reports bucket
      {
        Effect = "Allow"
        Action = [
          "s3:GetObject",
      ...
      # Inspect EventBridge rules/schedules
      {
        Effect = "Allow"
        Action = [
          "events:DescribeRule",
      ...
      # Ask Bedrock to explain deterministic compliance results
      {
        Effect = "Allow"
        Action = [
          "bedrock:InvokeModel"
...
resource "aws_iam_role_policy_attachment" "compliance_basic_execution" {
...
# Compliance Agent Lambda Role
# # need this for the Compliance Agent Lambda function
# resource "aws_iam_role" "compliance_role" {
#   name               = "${var.prefix}-compliance-role"
#   assume_role_policy = data.aws_iam_policy_document.lambda_assume_role.json
#   tags               = var.common_tags
...
# # Add the Compliance permissions
# # This gives the Compliance Lambda permission to inspect the security infrastructure, write compliance evidence, use the report bucket, and invoke Bedrock.
# resource "aws_iam_role_policy" "compliance_policy" {
#   name = "${var.prefix}-compliance-policy"
#   role = aws_iam_role.compliance_role.id
...
#       # Inspect DynamoDB evidence sources
#       {
#         Effect = "Allow"
#         Action = [
#           "dynamodb:DescribeTable",
...
#       # Bedrock explains compliance results
#       {
#         Effect = "Allow"
#         Action = [
#           "bedrock:InvokeModel"
