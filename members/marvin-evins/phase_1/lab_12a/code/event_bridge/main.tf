...
resource "aws_cloudwatch_event_target" "soar_reasoning_target" {
  rule      = aws_cloudwatch_event_rule.reasoning_completed_rule.name
...
resource "aws_cloudwatch_event_target" "soar_response_target" {
  rule      = aws_cloudwatch_event_rule.reasoning_completed_rule.name
  target_id = "SOARResponse"
  arn       = var.soar_response_lambda_arn
}

resource "aws_cloudwatch_event_rule" "reasoning_completed_rule" {
  name = "${var.prefix}-reasoning-completed-rule"

  event_pattern = jsonencode({
    source      = ["armageddon.soar-reasoning"]
    detail-type = ["ReasoningCompleted"]
  })
